...
export const IndexModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const { index, indexChange24h, indexHistory, indexDailyHistory, indexDailyHigh, indexDailyLow, language } = useMarketStore();
  const [chartType, setChartType] = useState<'intraday' | 'daily'>('intraday');

  const t = {
    title: language === 'zh' ? 'CR加權指數 (CR Index)' : 'CR Weighted Index',
    ...
    desc: language === 'zh' 
      ? 'CR加權指數是根據市場上30日交易量前50大的貨幣（權值幣）進行加權計算而得。起始點為 10,000.00 點，能即時反映整體虛擬貨幣市場的綜合表現與趨勢。' 
      : 'The CR Weighted Index is calculated based on the top 50 currencies by 30-day trading volume. Starting at 10,000.00 points, it reflects the overall performance and trend of the virtual currency market in real-time.',
  };

  const chartData = useMemo(() => {
    const data = chartType === 'intraday' ? indexHistory : indexDailyHistory;
    return data.map(h => {
  ...
  }, [indexHistory, indexDailyHistory, chartType, language]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-zinc-900/60 backdrop-blur-sm">
      <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col border border-zinc-200 dark:border-zinc-800 animate-in fade-in zoom-in-95 duration-200">
        
        {/* Header */}
        <div className="flex justify-between items-center p-6 border-b border-zinc-100 dark:border-zinc-800">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-indigo-600 text-white flex items-center justify-center font-bold text-xl shadow-md">
              CR
            </div>
            <div>
        ...
        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <StatCard label={t.high} value={formatPrice(indexDailyHigh)} />
              <StatCard label={t.low} value={formatPrice(indexDailyLow)} />
            </div>
          </div>
